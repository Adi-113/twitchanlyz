...
// Select & size the SVG
const svg = d3.select("#network").attr("viewBox", [0, 0, fullW, fullH]);
...
d3.json(rawDataURL)
  .then((raw) => {
    ...
    const sim = d3
      ...
      .force(
        "center",
        d3.forceCenter(width / 2 + margin.left, height / 2 + margin.top)
      )
      .force(
        "collide",
        d3.forceCollide().radius((d) => size(d.degree) + 10)
    ...
    function ticked() {
      ...
      nLabel.attr("x", (d) => d.x).attr("y", (d) => d.y);
    }
